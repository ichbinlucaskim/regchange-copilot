...
**하지 않는 일**: 침해사고 탐지·대응, 취약점 스캔, 개인정보 처리(설계상 배제), 사내 정책
자동 수정·배포, 감독당국 자동 통지, 법률 자문. 미구현이 아니라 **의도적인 범위 제외**다.
...
`enqueue_actions` 로 들어오는 **간선이** `human_review` **하나**이고, **DB 권한**이 `app_graph`
의 INSERT 를 막고, **FK** 가 승인 레코드 없는 `action_outbox` 행을 막는다.
`bump_revision` 은 간선이 남아 있으되 값 하나(`MAX_REVISIONS`)로 꺼져 있다 — 지우지 않은
이유는 되돌리기가 값 하나이기 때문이다(§2).
...
전자는 기술 선택의 자랑이고 후자는 업무 구조에 대한 주장이다.

킬 스위치 셋은 DB 에 있고 **새 환경은 전부 꺼진 채로 시작한다** — 설정 누락이 기능을
켜는 일이 없어야 한다 (ADR-019, fail-closed).
...
**Anthropic 은 프레임워크를 신중히 쓰라고 권고한다** —
[Building effective agents](https://www.anthropic.com/engineering/building-effective-agents):
추상화 계층이 프롬프트와 응답을 가려 디버깅을 어렵게 하고, 단순한 구성으로 충분한데도
복잡도를 더하고 싶게 만든다. **LLM API 를 직접 쓰는 것에서 시작하라.**
...
파라미터 · 입력 문서 버전 · 검색된 청크 ID · 원본 출력(저장소 키 + 해시) · 토큰 · 지연 ·
결과**를 남긴다. 실패와 재시도도 행으로 남는다 — **"실패한 호출"과 "하지 않은 호출"이 같은
부재가 되면 안 된다.** 프레임워크가 무엇을 보냈는지 우리가 항상 볼 수 있어야 한다.

**그리고 실제로 가정 하나가 깨졌다.** 체크포인터 직렬화기가 dataclass 의 `tuple` 을
`list` 로 되돌리는 것을 실측했고, "등록되지 않은 타입은 앞으로 차단된다"는 경고도 확인했다.
그래서 **그래프 상태에는 평범한 사전만 담는다**(`graph/state.py`). ADR-013 이 프레임워크에
대해 경계한 지점 — *"프레임워크 동작에 대한 잘못된 가정"* — 이 실제로 나타난 자리다.
...
틀렸다 — 타법개정은 LLM 3회가 아니라 2회를 부른다**(인용 0건이라 gate 3단이 돌지 않는다).
...
스캐너는 `str` 이 아니라 `UntrustedText` 를 받는다. 생성자가 모듈 private 토큰을
...
얹혀 1,261 이 되어 캐시된다. **미달은 오류가 아니라 조용한 무효과**이므로("No error is
...
사용자 메시지다 — 매 호출이 쓰기(1.25배)가 되고 읽기는 한 번도 일어나지 않아 **비용이
...
없다. **필요 없어서가 아니라 기준선을 지키려고** 안 만졌다 — 해제 조건은 기준선을 다시
잡는 것(골든셋 재실행 $3.97)을 함께 지출할 때다.
...
**재순위화(reranking)를 도입하지 않았다** — 순서가 아니라 **선택**이 병목이라는 측정 때문이다
...
→ **재검토 조건**: 검색 실패의 원인이 "틀린 조가 올라왔다"가 아니라 **"맞는 조인데 그 안의
...
**도구를 정의하지 않았다.** `LLMClient.complete` 의 서명에 `tools` 인자가 없다 — 계약
수준에서 없다.
...
기능적으로는 이미 나뉘어 있다 — 노드로. 도구와 노드의 차이는 누가 순서를 정하는가뿐이다.
```
...
그때도 학습 결과는 프롬프트가 아니라 **조회 가능한 규칙**으로 둔다 — 감사에서 "왜 이
부서인가"에 답해야 한다. 지금 반려 사유는 행으로만 남고 아무것도 그것을 읽지 않는다.
...
**출력 텍스트와 검색된 원문만** 받는다. 이것은 멀티 에이전트가 아니라 **컨텍스트 분리**이며,
같은 모델을 다른 프롬프트로 부르는 것으로 충분하다.

→ **재도입 조건**: **관점별 병렬 분석이 필요할 때** — 정보보호 관점 vs 개인정보 관점
(기획서 4.3의 「관점별 필터」). 그때는 §2의 Parallelization 조건과 같은 신호다.

**그리고 오류 누적을 늘리지 않는 것이 이 절의 요지다.** 승인 게이트 전에 여러 턴이 쌓이면
**검토자가 검증해야 할 표면이 커진다.** 3턴짜리 추론을 검증하려면 3턴을 전부 읽어야 하고,
그러면 검토자는 결론만 보고 승인하게 된다 — ADR-003 이 「틀렸음을 알게 되는 신호 2번」으로
적은 상태(승인의 형식화)가 구조적으로 유도된다.
...
내놓기 때문이며, 능력이 아니라 무능의 부작용이다. 기권은 적중·정밀도와 함께 읽어야 한다.
...
### 9-4. Ablation — 컴포넌트를 빼거나 바꿔 기여를 쟀다
...
**위 넷은 컴포넌트 하나씩을 빼거나 바꿔 기여를 잰 것이다.** 아래 둘은 같은 목적이되
방식이 다르다 — 모순 필터는 **전수 관측**으로(21건 중 12건 발화, 그중 10건이 정답 출력),
라우팅은 **기록과 결정론적 검색만 읽어**(LLM 0회 · $0) 기각했다.

### 9-5. 변수 통제 — 대조가 성립하려면 변수가 하나여야 한다
...
조정 가능한 지점 **76개** 중 **측정으로 정한 것은 15개**다. 나머지는 추론 17 · 관행 15 ·
설계 29 — **5분의 1만 실측으로 정해져 있다**는 것이 현재 상태다 ([docs/17](./docs/17-engineering-knobs.md)).
...
**「만들고 안 썼다」가 「안 만들었다」보다 강하다.** 전자는 재본 것이고 후자는 안 재본 것이다.
아래 수치는 **골든셋 15건의 최종 출력** 기준이다(라우팅만 42건 기록 기반).
...
1.0000 이다.** 목표 0.95 를 넘긴 것이 아니라 **코드가 도는 것을 확인한 항등식**이며,
정보를 가진 값은 비율이 아니라 폐기가 몇 번 발화했는가다 (영향평가 단계에서 42건 중
...
case-013·042 는 **설계된 decoy 문단을 인용한 것 자체가 실패**인데, 「이 문단이 이 주장을
뒷받침하는가」만 물으면 주장이 문단을 정확히 서술하므로 `SUPPORTED` 다.
**척도가 「인용이 주장을 받치는가」는 묻고 「그 인용을 했어야 하는가」는 묻지 않는다**
([docs/23 §2.8](./docs/23-metrics-summary.md)). 판정 기준이 **하한만 고정돼 있었다**는 것도
...
꺼지면 빈 결과가 아니라 **예외로 멈추고**, 멈춘 이유를 셋으로 구별한다 —
...
**AWS 가 아니라 로컬 cron(launchd)이다. 이유를 적는다.**
...
숫자의 근거는 로그가 아니라 `ops_run` 질의다 — **실패한 실행도 행으로 남는다.**
"실패한 날"과 "실행하지 않은 날"은 다른 사실이며 조치도 다르다.
...
7일차 점검에서 이 키가 `.env` 에 통째로 빠져 있었다 —
**경고는** `settings.py` **에 적혀 있었고 실행이 따르지 않았다**
([docs/24 §6](./docs/24-operations-record.md)).
...
`.env` **가 아니라 DB 에 있다** (ADR-019). 새 환경은 세 스위치가 **전부 꺼진 상태**로
...
쓸 수 없으며, **그 사실을** `tests/security/` **가 검사한다** (원칙 5).
