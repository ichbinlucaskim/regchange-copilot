...
| **Gate (코드 검사)**      | **채택**                       | 중간 검사를 **코드가 한다.** gate 2단은 순수 함수이고 LLM 에게 묻지 않는다 — 검증을 LLM 에 맡기면 gate 가 아니라 또 하나의 생성 단계가 된다                                                                                           |
...
만들어지지 않으므로, trusted 텍스트가 스캐너에 도달하는 경로가 타입 수준에서 없다.
...
**프롬프트는 전부 모듈 상수라 캐시 접두부가 매번 같다.**

**캐시되는 범위는 시스템 지침만이 아니다.** `citation-grounding` 의 지침은 1,005 토큰으로
Sonnet 5 의 최소 길이 1,024 에 19 토큰 모자란데, 응답 스키마가 얹혀 1,261 이 되어 캐시된다.
지침만 셌다면 "호출이 가장 많은 프롬프트는 캐시가 안 된다"가 결론이었을 것이다.
미달해도 오류가 나지 않으므로 이 사실은 `cache_read_input_tokens` 로만 드러난다.

**캐시 지점은 `system` 에만 둔다.** 자동 캐싱은 마지막 블록에 거는데, 우리 요청의 마지막은
케이스마다 달라지는 `user` 메시지다. 매 호출이 쓰기(1.25배)가 되고 읽기는 일어나지 않는다.

실측 효과는 입력 비용 −37.7%, 실행 전체 −10.8% 다. 출력이 비용의 71%라 총액 절감은
여기서 막힌다.

**3단계 기준선 이후 프롬프트 본문을 동결했다.** 고치면 그 시점부터 이전 출력과 비교할 수
없다. 해제 조건은 기준선을 다시 잡는 비용(골든셋 재실행 $3.97)을 함께 낼 때다.
...
(LLM 이 재현율을 0.8167 → 0.4583 으로 깎는다).

다만 **42건에서 그 판단이 흔들렸다**(§12). 신규 27건은 반대 방향이었고(검색이 병목),
어느 분포가 운영에 가까운지를 인용 밀도 대조로 갈랐다(개정 조문 0.3750 vs 사내 0.1447,
판정 폭의 2.63배). 기준을 측정 전에 고정해 두었기에 판정이 가능했다.
...
먼저 돌려본다. 호출 횟수를 코드가 정하면 gate 2단·비용 예측·변동성이 그대로 유지되고,
모델에게 맡기면 셋 다 깨진다. 순서를 지키는 것이 이 저장소의 규칙이다.
...
LLM 의 EASY 도 정확히 같은 0.625 다.** 베이스라인을 측정하지 않았으면 「EASY 재현율 1.0000」을
